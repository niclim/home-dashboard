...
@routes.route("/items/all", methods=["GET"])
def get_all_items():
...
@routes.route("/item/status", methods=["GET"])
def get_status():
    # Get parameter from the URL
    item_name = request.args.get("name")

    # Get items from the helper
    status = helper.get_status(item_name)
...
@routes.route("/item/update", methods=["PUT"])
def update_status():
    # Get item from the POST body
    req_data = request.get_json()
    item = req_data["item"]
...
@routes.route("/item/remove", methods=["DELETE"])
def delete_item():
    # Get item from the POST body
    req_data = request.get_json()
    item = req_data["item"]
